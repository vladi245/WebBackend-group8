...
async function getDeskById(id) {
    const res = await pool.query('SELECT * FROM desk WHERE id = $1', [id]);
...
async function createDesk({ id, height }) {
    ...
    const res = await pool.query('INSERT INTO desk (id, height) VALUES ($1, $2) RETURNING *', [id, height]);
    return res.rows[0] || null;
}

async function updateDeskHeight(id, height) {
    const res = await pool.query('UPDATE desk SET height = $2 WHERE id = $1 RETURNING *', [id, height]);
    return res.rows[0] || null;
}
